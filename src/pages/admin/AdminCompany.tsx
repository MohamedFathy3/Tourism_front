...
import { companyAdminService } from "@/services/admin";
...
const AdminCompany = () => {
  ...
  const {
    ...
    refresh,
    changePerPage,
  } = useAdminResource(companyAdminService, { 
    perPage: 5,
    initialFilters: {},
    orderBy: 'id',
  ...
  return (
    <>
      <AdminTable
        data={companies}
        columns={TableConfigs.company.columns}
        actions={actions}
        loading={loading}
        error={error}
        onRefresh={refresh}
        onAdd={handleAdd}
        addLabel={lang === 'ar' ? 'إضافة شركة' : 'Add Company'}
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>
                  {editingItem 
                    ? (lang === 'ar' ? '✏️ تعديل الشركة' : '✏️ Edit Company')
