...
import { contactAdminService } from "@/services/admin";
...
const AdminContact = () => {
  ...
  const {
    ...
    refresh,
    changePerPage,
  } = useAdminResource(contactAdminService, { 
    perPage: 5,
    initialFilters: {},
    orderBy: 'id',
  ...
  const handleDelete = async (item: any) => {
    if (confirm(lang === 'ar' ? `حذف بيانات التواصل؟` : `Delete contact data?`)) {
  ...
  return (
    <>
      <AdminTable
        data={contacts}
        columns={TableConfigs.contact.columns}
        actions={actions}
        loading={loading}
        error={error}
        onRefresh={refresh}
        ...
        searchable={true}
        onSearch={(query) => setFilter('email', query)}
        searchPlaceholder={lang === 'ar' ? 'بحث في بيانات التواصل...' : 'Search contacts...'}
      />
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>
                  {editingItem 
                    ? (lang === 'ar' ? '✏️ تعديل بيانات التواصل' : '✏️ Edit Contact')
                    : (lang === 'ar' ? '➕ إضافة بيانات تواصل جديدة' : '➕ Add New Contact')}
                </h2>
                <button
                  onClick={() => {
                    setShowModal(false);
