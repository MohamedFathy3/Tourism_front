...
import { projectAdminService } from "@/services/admin";
...
const AdminProjects = () => {
  ...
  const {
    ...
    refresh,
    changePerPage,
  } = useAdminResource(projectAdminService, { 
    perPage: 5,
    initialFilters: {},
    orderBy: 'id',
  ...
  return (
    <>
      <AdminTable
        data={projects}
        columns={TableConfigs.project.columns}
        actions={actions}
        loading={loading}
        error={error}
        onRefresh={refresh}
        onAdd={handleAdd}
        addLabel={lang === 'ar' ? 'إضافة مشروع' : 'Add Project'}
        ...
        searchPlaceholder={lang === 'ar' ? 'بحث في المشاريع...' : 'Search projects...'}
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>
                  {editingItem 
                    ? (lang === 'ar' ? '✏️ تعديل المشروع' : '✏️ Edit Project')
